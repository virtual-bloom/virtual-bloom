...
public class Tela_Jogo3 {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Tela_Jogo3 window = new Tela_Jogo3();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(Tela_Jogo3.class.getResource("/resourses/Logo_Icon.png")));
		...
		camposLetras = new JTextField[palavra.length()];
	        for (int i = 0; i < palavra.length(); i++) {
	            ...
	            camposLetras[i].setBounds(65 + (i * 50), 220, 40, 50);
		...
		lbFundo.setIcon(new ImageIcon(Tela_Jogo3.class.getResource("/resourses/Tela_Jogo.png")));
        ...
        parte.setIcon(new ImageIcon(Tela_Jogo3.class.getResource(imagem)));
	...
	private void verificarVitoria() {
		...
		if (venceu) {
			JOptionPane.showMessageDialog(frame, "Você zerou o jogo! Ao clicar em ok você retornará a fase 1!");
		 	frame.dispose(); // Fecha a tela atual
		    Tela_Jogo telaJogo = new Tela_Jogo(); // Cria uma nova instância da Tela_Jogo1
		    telaJogo.frame.setVisible(true);
		}
	    }
		 ...
		 private void reiniciarJogo() {
			 // Reseta as variáveis
			    tentativasErradas = 0;
			    palavra = "CRISANTEMO"; // Ou você pode escolher uma nova palavra aleatória aqui
